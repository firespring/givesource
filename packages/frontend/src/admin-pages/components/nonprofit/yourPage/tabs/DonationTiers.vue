    ...
    <table class="js-table-reorder">
      ...
      <tbody class="ui-sortable">
        <donation-tiers-list-table-row
          v-for="(donationTier, index) in donationTiers"
          :key="donationTier.id"
          :index="index"
          :amount="donationTier.amount"
          :description="donationTier.description"
          @change="changeDonationTier"
        />
      </tbody>
    </table>
...
export default {
  ...
  methods: {
    ...
    updateDonationTiers: function () {
      ...
      const created = _.filter(current, function (tier) {
        return !tier.hasOwnProperty('id') && tier.amount !== 0
      })

      const deleted = []
      const updated = []
      const changed = _.differenceWith(original, current, _.isEqual)
