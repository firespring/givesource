...
describe('PatchDonation', function () {
  it('should return an updated donation', async function () {
    const original = await TestHelper.generate.model('donation')
    const updated = await TestHelper.generate.model('donation', { uuid: original.uuid })
    sinon.stub(DonationsRepository.prototype, 'get').resolves(original)
    const upsertStub = sinon.stub(DonationsRepository.prototype, 'upsert').resolves(updated)

    const body = { someUpdatedParam: 'updated' }
    const result = await TestHelper.callApi(PatchDonation, {}, null, { body })
  ...
  it('should return error on exception thrown - get', async function () {
    ...
    const response = TestHelper.callApi(PatchDonation)
  ...
  it('should return error on exception thrown - save', async function () {
    ...
    sinon.stub(DonationsRepository.prototype, 'get').resolves(original)
    sinon.stub(DonationsRepository.prototype, 'upsert').rejects(errorStub)
